...
#Dropdown widget
domains_sorted = sorted(df_cat['domains'])
menu = list(zip(domains_sorted, domains_sorted))
selection = Select(options=menu)
...
def callback_remove():
    new_plot = figure(tooltips = HOVER_TOOLTIPS,
              ...
              plot_width=1000, 
              title=title)
    cat_selected = 'Teaching'
    df.dropna(inplace=True)
    source =  df[df["target_category"].str.contains(cat_selected)==False]
    source = source[source['source_category'].str.contains(cat_selected)==False]
    G = networkx.from_pandas_edgelist(source, 'source', 'target', edge_attr=['weight'], create_using=networkx.DiGraph())
...
layout = column(group, row(remove_school_button,reset_button, selection,sizing_mode="scale_width"), plot)
...
analysis = column(analysis_text, density_text, density, 
...
Title_text = Div(text= "Network of Innovation Units at Penn", style={'font-size': '200%'})
dashboard = column(Title_text, row(layout, analysis))
